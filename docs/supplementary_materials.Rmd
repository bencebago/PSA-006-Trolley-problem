...
trolley <- read.csv("data/trolley.csv")

study1a <- filter(trolley, include_study1a)
study1a_ne <- filter(trolley, include_study1a_withoutfamiliarity)
...
trolley_raw %>% 
  distinct(lab) %>% 
  mutate(lab = case_when(lab == "USA_087?id=4012" ~ "USA_087",
                         TRUE ~ lab)) %>%
  drop_na(lab) %>% 
  count()
...
# Proportion of responses after all exclusion
round(n_after_all_exclusion / nrow(trolley_raw) * 100, 2)
...
setwd("~/PSA-006-Trolley-problem")
cd_raw <- read_csv("data/collectivism_similarity_matrix-from_culturaldistance.com.csv")
...
tabledata_s2a_ne[i, "Raw effect"]=(mean(dat_long[dat_long$personal_force==0 & dat_long$intention==0,]$rate,na.rm=T) - mean(dat_long[dat_long$personal_force==1 & dat_long$intention==0,]$rate,na.rm=T)) - (mean(dat_long[dat_long$personal_force==0 & dat_long$intention==1,]$rate,na.rm=T) - mean(dat_long[dat_long$personal_force==1 & dat_long$intention==1,]$rate,na.rm=T)) 
...
tabledata_s2b_ne[i, "Raw effect"]=(mean(dat_long[dat_long$personal_force==0 & dat_long$intention==0,]$rate,na.rm=T) - mean(dat_long[dat_long$personal_force==1 & dat_long$intention==0,]$rate,na.rm=T)) - (mean(dat_long[dat_long$personal_force==0 & dat_long$intention==1,]$rate,na.rm=T) - mean(dat_long[dat_long$personal_force==1 & dat_long$intention==1,]$rate,na.rm=T)) 
