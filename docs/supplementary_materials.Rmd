...
# exclusions <-
  trolley %>% 
  nest(data = everything()) %>% 
  mutate(all_responses = map(data, ~count(.x, Region, 
                                              name = "all", .drop = TRUE)),
         exclude_careless = map(data, ~filter(.x, !include_nocareless) %>% 
                                      group_by(Region) %>% 
...
source(here("R/calculate_study1_stat.R"))
...
source(here("R/calculate_study2_stat.R"))
