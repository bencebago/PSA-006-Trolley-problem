...
create_table <- function(data = NULL){
  ...
  fmt_number(columns = vars(`Age Mean`, `Age SD`),
             decimals = 1) %>% 
  tab_spanner(columns = vars(data), label = "Region") %>%
  ...
  fmt_percent(columns = vars(`Male %`, `Higher education %`),
              decimals = 1) %>% 
  fmt_missing(vars(Country, Collectivism, Region, data, `Higher education %`, `Male %`, `Age SD`)) %>% 
...
```{r}
study_files <- list.files(path = here::here("data/"), 
                          pattern = "study..")

summaries <-
  # Read data
  tibble(study = study_files) %>% 
  mutate(data = map(study, ~read_csv(here::here("data/", .x)))) %>% 
  # Process and summarise data on 3 levels (country, region, all)
  mutate(proc_data = map(data, process_study),
         country_sum = map(proc_data, ~group_by(.x, Region, country3) %>% 
                                       summarise_study() %>% 
                                       left_join(cd, by = "country3") %>% 
                                       add_column(data = "By country", .before = 1) %>% 
         ...
         region_sum = map(proc_data, ~group_by(.x, Region) %>% 
                                       summarise_study() %>% 
                            add_column(data = "By region", .before = 1) %>% 
                            arrange(Region)),
         all_sum =map(proc_data, ~summarise_study(.x) %>% 
                        add_column(data = "All", .before = 1) %>% 
                        mutate(Region = "All")),
...
# Save
gtsave(summaries$gtable[[1]], "docs/table1_study1a.rtf")
gtsave(summaries$gtable[[2]], "docs/table1_study1b.rtf")
gtsave(summaries$gtable[[3]], "docs/table1_study2a.rtf")
gtsave(summaries$gtable[[4]], "docs/table1_study2b.rtf")

# TODO: Iran higher education? it seems to be NA
# Make summaries using the gt summary functions instead of binding rows
...
trolley_proc <- vroom::vroom(here::here("data/trolley_preprocessed.csv"))
