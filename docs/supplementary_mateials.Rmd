...
trolley_raw <- read.csv(here("data/trolley_raw.csv"))
trolley_proc <- read.csv(here("data/trolley_preprocessed.csv"))
trolley <- read_csv(here("data/trolley.csv"), guess_max = 1e5)
...
trolley_proc_familiarity <- filter(trolley, include_allstudy_withoutfamiliarity)
```
...
datt=lmBF(rate ~ personal_force*intention, data=dat_long, rscaleFixed=0.19)
datt2=lmBF(rate ~ personal_force+intention, data=dat_long)
datt3=as_tibble(datt/datt2)
...
for (i in levels(factor(trolley_proc$country3))){
  d=subset(study2b, country3==i, select=c("trolley_3_rate", "trolley_4_rate", "trolley_5_rate", "trolley_6_rate"))
...
```{r familiarity, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
#no exclusion
#use data study1a
tabledata_s2a_ind = data.frame(Effect=c(rep("Interaction", 3)),Dilemma=c("Trolley", "Trolley", "Trolley"),Cluster=c("Eastern", "Southern", "Western"), matrix(nrow=3, ncol=3, dimnames=list(survey,c("BF", "b",  "p"))))

for (i in survey){
  
dat=subset(trolley_proc, survey==i & familiarity==4|familiarity,  select=c("trolley_3_rate", "trolley_4_rate", "trolley_5_rate", "trolley_6_rate", "country3", "familiarity"))
dat_long=na.omit(melt(data=dat, variable.name = "condition", value.name="rate", id.vars=c("familiarity", "country3")))
dat_long$personal_force = factor(ifelse(dat_long$condition=="trolley_3_rate"|dat_long$condition=="trolley_5_rate", 0, 1))
dat_long$intention = factor(ifelse(dat_long$condition=="trolley_4_rate"|dat_long$condition=="trolley_5_rate", 1, 0))
dat_long$country3=as.character(dat_long$country3)
dat_long$country3=factor(paste0("0", dat_long$country3))

  datt=lmBF(rate ~   personal_force*intention*familiarity+ country3, data=dat_long, whichRandom="country3",rscaleCont = c("personal_force:intention:familiarity" = 0.19))
  datt2=lmBF(rate ~  personal_force+intention+familiarity+personal_force:familiarity+personal_force:intention+intention:familiarity+country3, whichRandom="country3",data=dat_long)  
  frequentist_m=lmer(rate ~ personal_force*intention*familiarity +(1|country3), data=dat_long)
  frequentist=broom.mixed::tidy(frequentist_m)
datt3=as_tibble(datt/datt2)
tabledata_s2a_ind[i, "BF"]=signif(datt3[1,1],4)
tabledata_s2a_ind[i, "b"]=round(frequentist[8, "estimate"],3)
...
tabledata_s2b_ind = data.frame(Effect=c(rep("Interaction", 3)), Dilemma=c("Speedboat", "Speedboat", "Speedboat"),Cluster=c("Eastern", "Southern", "Western"), matrix(nrow=3, ncol=3, dimnames=list(survey,c("BF", "b",  "p"))))

for (i in survey){
  
dat=subset(trolley_proc, survey==i,  select=c("speedboat_3_rate", "speedboat_4_rate", "speedboat_5_rate", "speedboat_6_rate", "country3", "familiarity"))
dat_long=na.omit(melt(data=dat, variable.name = "condition", value.name="rate", id.vars=c("familiarity", "country3")))
dat_long$personal_force = factor(ifelse(dat_long$condition=="speedboat_3_rate"|dat_long$condition=="speedboat_5_rate", 0, 1))
dat_long$intention = factor(ifelse(dat_long$condition=="speedboat_4_rate"|dat_long$condition=="speedboat_5_rate", 1, 0))
dat_long$country3=as.character(dat_long$country3)
...
tabledata_s1b_ind = data.frame(Effect=c(rep("Personal Force", 3)),Dilemma=c("Speedboat", "Speedboat", "Speedboat"),Cluster=c("Eastern", "Southern", "Western"), matrix(nrow=3, ncol=3, dimnames=list(survey,c("BF", "b",  "p"))))

for (i in survey){
  
dat=subset(trolley_proc, survey==i,  select=c("speedboat_1_rate", "speedboat_2_rate",  "country3", "familiarity"))
dat_long=na.omit(melt(data=dat, variable.name = "condition", value.name="rate", id.vars=c("familiarity", "country3")))
dat_long$personal_force = factor(ifelse(dat_long$condition=="speedboat_1_rate", 0, 1))
dat_long$country3=as.character(dat_long$country3)
dat_long$country3=factor(paste0("0", dat_long$country3))

  datt=lmBF(rate ~   personal_force*familiarity+ country3, data=dat_long, whichRandom="country3",rscaleCont = c("personal_force:familiarity" = 0.26))
  datt2=lmBF(rate ~  personal_force+familiarity+country3, whichRandom="country3",data=dat_long)  
  frequentist_m=lmer(rate ~ personal_force*familiarity +(1|country3), data=dat_long)
  frequentist=broom.mixed::tidy(frequentist_m)
datt3=as_tibble(datt/datt2)
...
tabledata_s1a_ind = data.frame(Effect=c(rep("Personal Force", 3)),Dilemma=c("Trolley", "Trolley", "Trolley"),Cluster=c("Eastern", "Southern", "Western"), matrix(nrow=3, ncol=3, dimnames=list(survey,c("BF", "b",  "p"))))

for (i in survey){
  
dat=subset(trolley_proc, survey==i,  select=c("trolley_1_rate", "trolley_2_rate",  "country3", "familiarity"))
dat_long=na.omit(melt(data=dat, variable.name = "condition", value.name="rate", id.vars=c("familiarity", "country3")))
dat_long$personal_force = factor(ifelse(dat_long$condition=="trolley_1_rate", 0, 1))
dat_long$country3=as.character(dat_long$country3)
dat_long$country3=factor(paste0("0", dat_long$country3))
...
full_familiarity=rbind(tabledata_s1a_ind, tabledata_s1b_ind, tabledata_s2a_ind, tabledata_s2b_ind)

rownames(full_familiarity) <- NULL
...
kbl(
  ...
  booktabs = T,
  escape=T,
  caption = "Did familiarity affect the results?",
  centering=T,
  position="H"
  )%>%
  kable_styling( full_width = F,  position="left")%>%
  row_spec(row=0, bold=TRUE)%>%
    collapse_rows(columns = 1, valign = "top", latex_hline = "major", row_group_label_position = "identity")%>%
    collapse_rows(columns = 2, valign = "top", latex_hline = "major", row_group_label_position = "identity")
