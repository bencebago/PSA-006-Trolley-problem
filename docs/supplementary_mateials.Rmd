...
# Import data
trolley_raw <- vroom(here("data/trolley_raw.csv"))
trolley_proc <- vroom(here("data/trolley_preprocessed.csv"))
study1a <- vroom(here("data/study1a.csv"))
study1b <- vroom(here("data/study1b.csv"))
study2a <- vroom(here("data/study2a.csv"))
study2b <- vroom(here("data/study2b.csv"))
```
...
In the following analysis we used Bayesian and frequentist mixed effect models in which we added the random intercept of countries to all models. In the Bayesian analysis, the bayes factor is a comparison of the model in which there are only main effects (denominator) and the full model (including the main effects and interaction). We can see that regarding the effect of personal force, there is a strong evidence that no individual or country level measure of culture affects it, in fact, in 4, out of the 5 cases in the speedboat, and 5 out of 5 cases in the trolley dilemma we have strong evidence for a null effect.
...
for (i in vars){
  if(i=="Collectivism"){
  datt=lmBF(rate ~  personal_force*Collectivism+country3, data=dat_long, whichRandom="country3",  rscaleCont=c( "personal_force:Collectivism"=0.26))
  datt2=lmBF(rate ~  personal_force+Collectivism+country3, data=dat_long, whichRandom="country3")
  frequentist_m=lmer(rate ~ personal_force*Collectivism +(1|country3), data=dat_long)
  frequentist=broom.mixed::tidy(frequentist_m)
  } else if(i=="ver_ind"){
  datt=lmBF(rate ~   personal_force*ver_ind+country3, data=dat_long, whichRandom="country3",  rscaleCont=c( "personal_force:ver_ind"=0.26))
  datt2=lmBF(rate ~   personal_force+ver_ind+country3, data=dat_long, whichRandom="country3")
  frequentist_m=lmer(rate ~ personal_force*ver_ind +(1|country3), data=dat_long)
  frequentist=broom.mixed::tidy(frequentist_m)
  }else if(i=="hor_ind"){
  datt=lmBF(rate ~   personal_force*hor_ind+country3, data=dat_long, whichRandom="country3", rscaleCont=c( "personal_force:hor_ind"=0.26))
  datt2=lmBF(rate ~   personal_force+hor_ind+country3, data=dat_long, whichRandom="country3")
  frequentist_m=lmer(rate ~ personal_force*hor_ind +(1|country3), data=dat_long)
  frequentist=broom.mixed::tidy(frequentist_m)
  }else if(i=="ver_col"){
  datt=lmBF(rate ~   personal_force*ver_col+country3, data=dat_long, whichRandom="country3",  rscaleCont=c( "personal_force:ver_col"=0.26))
  datt2=lmBF(rate ~   personal_force+ver_col+country3, data=dat_long, whichRandom="country3")
  frequentist_m=lmer(rate ~ personal_force*ver_col +(1|country3), data=dat_long)
  frequentist=broom.mixed::tidy(frequentist_m)
  }else if(i=="hor_col"){
  datt=lmBF(rate ~   personal_force*hor_col+country3, data=dat_long, whichRandom="country3",  rscaleCont=c( "personal_force:hor_col"=0.26))
  datt2=lmBF(rate ~  personal_force+hor_col+country3, data=dat_long, whichRandom="country3")
  frequentist_m=lmer(rate ~ personal_force*hor_col +(1|country3), data=dat_long)
  frequentist=broom.mixed::tidy(frequentist_m)
  }
datt3=as_tibble(datt/datt2)
tabledata_s1a_ind[i, "BF"]=signif(datt3[1,1],4)
tabledata_s1a_ind[i, "b"]=round(frequentist[4, "estimate"],3)
tabledata_s1a_ind[i, "p"]=round(frequentist[4, "p.value"],3)
graphdata[i, "b"]=round(frequentist[4, "estimate"],3)
graphdata[i, "lower"]=round(confint(frequentist_m),2)[6,1]
graphdata[i, "higher"]=round(confint(frequentist_m),2)[6,2]
...
```{r study1a vertical horizontal individualism,  out.width="80%", fig.cap="Personal level individualism/collectivism effects on the Trolley dilemmas. Beta effect size of the interaction ", echo =FALSE,message=FALSE, warning=FALSE, paged.print=FALSE}
...
datt3=as_tibble(datt/datt2)
tabledata_s1b_ind[i, "BF"]=signif(datt3[1,1],4)
...
<!-- Bayesian personal level individualism  w/o exclusion-->
```{r study1b vertical horizontal individualism,  out.width="80%", fig.cap="Personal level individualism/collectivism effects on the Speedboat dilemmas", echo =FALSE,message=FALSE, warning=FALSE, paged.print=FALSE}
#line plot, two conditions are different lines and y = acceptance, x individualism, should do it with allcol/ind scales.
graphdata %>% 
  ggplot() +
  aes(
...
table_data <- 
  ...
  drop_na(lab) %>% 
  transmute(country3 = str_extract(lab, "[A-Z]+") %>% 
...
table_data <- 
  table_data %>% 
  pivot_wider(names_from = region, values_from = Country)
